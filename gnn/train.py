...
parser.add_argument('--graph_dim', type=str, default='128,32,16', help='lstm and graph layer dimension')
...
def train(seed):
    ...
    for cross_valid in range(5):
                ...
                for label in label2id:
                    ...
                    if data.output.view(-1)[y_pred[labelid]] == labelid:
                        ...
                        if output_file != None:
                            ...
                            word = data.words[data.sents[int(id/sent_len)][id%sent_len]]
        ...
        optimizer = torch.optim.Adam(
            filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr/float(batch), weight_decay=args.wd)
